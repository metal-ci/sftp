...
func benchmarkWrite(b *testing.B, bufsize int, delay time.Duration) {
	...
	for i := 0; i < b.N; i++ {
		...
		defer vfs.Remove(f.Name()) // actually queue up a series of removes for these files
...
func benchmarkCopyDown(b *testing.B, fileSize int64, delay time.Duration) {
	...
	defer vfs.Remove(srcFilename)
	zero, err := vfs.Open("/dev/zero")
	if err != nil {
		b.Fatal(err)
	}
